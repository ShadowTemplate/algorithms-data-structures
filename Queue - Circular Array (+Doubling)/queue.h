...
public:
	...
	int size(); // returns the number of element in the queue
...
bool Queue<T, P>::operator ==(Queue<T, P>& q2)
{
	...
	for (typename std::vector<T>::iterator it = q1elem.begin(); it != q1elem.end(); it++)
		this->enqueue(*it);

	for (typename std::vector<T>::iterator it2 = q2elem.begin(); it2 != q2elem.end(); it2++)
		q2.enqueue(*it2);
