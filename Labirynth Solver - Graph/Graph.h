...
public:
	...
	bool operator ==(const Graph<N, L, W>&) const;
	bool operator !=(const Graph<N, L, W>&) const;
...
template<class N, class L, class W>
bool Graph<N, L, W>::operator ==(const Graph<N, L, W>& g2) const
{
	std::stringstream graph1, graph2;

	graph1 << *this;
	graph2 << g2;

	return (graph1.str() == graph2.str());
}

template<class N, class L, class W>
bool Graph<N, L, W>::operator !=(const Graph<N, L, W>& graph2) const
{
	return (!(*this == graph2));
}
...
bool Graph<N, L, W>::connectedGraph() const
{
	 ...
	 * call daisyChained on that node;
	 * return daisyChained.size() == nodesList.size()
	 */
}
