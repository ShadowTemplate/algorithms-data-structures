...
public:
	...
	vector<value_type> elementsArray() const; // return a vector with the values of the nodes in the tree
...
private:
	...
	void eraseLeaves(node, const value_type&); // erases the leaves having a certain value from the subtree having root in the node
...
{
	if ( n1 == 0 || n2 == 0 )
		throw std::logic_error("BinTree (exception) - Unable to swap nodes (invalid node)");
...
bool BinTree<T, N>::hasSubTree(const BinTree<T, N>& t2)
{
	...
	//1st !empty >= 2nd (>1)
	vector<node> nodes = this->findOccurrences(t2.read(t2.root())); // get all the nodes having the same value

	bool flag = 0;
	for (typename vector<node>::iterator it = nodes.begin(); it != nodes.end() && !flag; it++)
		flag = flag + hasSubTree(*it, t2, t2.root()); // check if there is the parameter subtree at least starting from one node
...
{
	if (this->empty())
		throw std::logic_error("BinTree (exception) - Unable to perform preorder visit (empty tree)");
...
{
	if (this->empty())
		throw std::logic_error("BinTree (exception) - Unable to perform inorder visit (empty tree)");
...
{
	if (this->empty())
		throw std::logic_error("BinTree (exception) - Unable to perform postorder visit (empty tree)");
...
{
	if (this->empty())
		throw std::logic_error("BinTree (exception) - Unable to perform breadth visit (empty tree)");
