...
ostream& operator <<(ostream& os, Stack<T, N>& currstack)
{
	...
	else
	{
		...
		for (typename deque<T>::const_reverse_iterator it = elements.rbegin(); it != elements.rend(); it++)
			currstack.push(*it);
	}
...
bool Stack<T, N>::operator ==(Stack<T, N>& s2)
{
	...
	for (typename deque<value_type>::const_reverse_iterator it = s1elem.rbegin(); it != s1elem.rend(); it++)
		this->push(*it);

	for (typename deque<value_type>::const_reverse_iterator it2 = s2elem.rbegin(); it2 != s2elem.rend();
			it2++)
		s2.push(*it2);
...
int Stack<T, N>::size()
{
	...
	for (typename deque<value_type>::const_reverse_iterator it = elements.rbegin(); it != elements.rend();
			it++)
		this->push(*it);
