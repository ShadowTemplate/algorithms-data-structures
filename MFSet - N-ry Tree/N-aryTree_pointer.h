...
template<class T>
class NaryTree_pointer: public NaryTree<T, CellNT<T>*>
{
public:
	typedef typename NaryTree<T, CellNT<T>*>::value_type value_type;
	typedef typename NaryTree<T, CellNT<T>*>::node node;
...
{
	this->create();
	node newroot = new CellNT<value_type>(value);
...
NaryTree_pointer<T>& NaryTree_pointer<T>::operator =(const NaryTree_pointer<T>& tree)
{
	if (&tree != this) // avoid auto-assignment
	{
		...
		if (tree.root() != 0)
		{
			node newroot = new CellNT<value_type>;
			this->ins_root(newroot);
...
{
	if (n == 0)
		tree_root = new CellNT<value_type>;
	else
		tree_root = new CellNT<value_type>(n->getValue());
}
...
template<class T>
typename NaryTree<T, CellNT<T>*>::value_type NaryTree_pointer<T>::read(const node& n) const
{
	if (n == 0)
		throw std::logic_error("NaryTree_pointer (exception)");
...
void NaryTree_pointer<T>::insFirstChild(node n)
{
	...
	node son = new CellNT<value_type>;
...
void NaryTree_pointer<T>::insRightSibling(node n)
{
	...
	{
		node brother = new CellNT<value_type>;
...
void NaryTree_pointer<T>::build(NaryTree_pointer<T> t1, NaryTree_pointer<T> t2)
{
	...
	node newroot = new CellNT<value_type>(value_type());
	this->ins_root(newroot);
	node temp = this->root();
