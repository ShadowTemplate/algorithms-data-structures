...
public:
	...
	bool operator ==(const Set_pointerOrd<T>&) const; // hide this method to let Set_pointerOrd class use Set<T> operator ==
	bool operator !=(const Set_pointerOrd<T>&) const; // hide this method to let Set_pointerOrd class use Set<T> operator !=

	template <class E>
	friend ostream& operator <<(ostream& os, const Set_pointerOrd<E>&); // operator << with CONST set
...
{
	if (!set.findOrd(a))
		throw std::logic_error("Set_pointerOrd: exception - Unable to erase (element not in the set)");
...
{
	if(set.empty())
		throw std::logic_error("Set_pointerOrd: exception - Unable to pick an element (empty set)");
