...
public:
	...
	void epurateLeaves(const value_type&); // erases the leaves having the same value of the function
	...
	//vector<node> numChildKlevel(int) const; // return the nodes in the level K of the tree
...
private:
	...
	void epurateLeaves(const value_type&, node&); // erases the leaves having a certain value from the subtree having root in the node
};
...
int NaryTree<T, N>::size(const node n) const
{
	...
	{
		curr = nodes.front();
		num = num + numChild(curr);

		if (!is_leaf(curr))
		{
			node temp = firstChild(curr);
			for (int i = 0; i < numChild(curr) - 1; i++)
			{
...
bool NaryTree<T, N>::find(const value_type& val) const
{
	...
	for (typename std::vector<value_type>::iterator it = elements.begin(); it != elements.end(); it++)
		if (*it == val)
			return true;
...
template<class T, class N>
void NaryTree<T, N>::epurateLeaves(const value_type& val)
{
	if (this->empty())
		throw std::logic_error("BinTree (exception)");

	node n = this->root();
	this->epurateLeaves(val, n);
}
...
/*
 template<class T, class N>
 vector<N> NaryTree<T, N>::numChildKlevel(int k) const
 {
 node temp = 0;
 std::deque<node> nodes;
 int counter = 1;
 ...
 {
 if( counter == k)
 for(typename std::deque<node>::iterator it = nodes.begin(); it != nodes.end(); it++)
 nodesarray.push_back(*it);
...
{
	if (this->empty())
		throw std::logic_error("NaryTree (exception)");
...
bool NaryTree<T, N>::checkEquality(const N& n1, const NaryTree<T, N>& t2, const N& n2) const
{
	...
	if (!this->is_leaf(n1) && !t2.is_leaf(n2))
	{
		...
		{
			if (checkEquality(temp1, t2, temp2))
			{
		...
		return checkEquality(temp1, t2, temp2);
	}
...
template<class T, class N>
void NaryTree<T, N>::epurateLeaves(const value_type& val, node& n)
{
	if (!this->is_leaf(n))
	{
		node curr = this->firstChild(n);
